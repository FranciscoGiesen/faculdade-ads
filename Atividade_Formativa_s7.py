...
turmas = []
matriculas = []
def salvar(nome_arquivo,lista):
    with open (nome_arquivo,'w', encoding='utf-8') as arquivo:
        json.dump(lista,arquivo, indent=4)
def recuperar(nomedoarquivo,listadestino):
    try:
        with open (nomedoarquivo,'r', encoding='utf-8') as arquivo:
            dados = json.load(arquivo)
            listadestino.extend(dados)
...
Informe a opção desejada: '''))
            nomedoarquivo = nomedomenu + '.json'
            if opcao == 1:
                recuperar(nomedoarquivo,sobremenu)
                incluir(sobremenu, nomeminusculo)
                salvar(nomedoarquivo, sobremenu)
